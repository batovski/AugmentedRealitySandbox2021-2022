...
public class SumoCreator : MonoBehaviour
{
    ...
    private void Start()
    {
        Traci = GetComponent<TraciController>();
    ...
    public void LoadNetwork()
    {
        ...
        if (files != null)
        {
            ...
            if (NET_FILE != null)
            {
                ...
                InterfaceManager.Instance.HideMainMenuUI();
            }
            if (POLY_FILE != null)
            {
    ...
    private void StartSumo(string ConfigFile)
    {
        ...
        {
            Traci.Port = 80;
            Traci.HostName = Dns.GetHostEntry("localhost").AddressList[1].ToString();
            Traci.ConfigFile = ConfigFile;
            Traci.Invoke("ConnectToSumo", 0.0f);             
        }
        catch (Exception e)
        {
            UnityEngine.Debug.LogError(e.Message);
